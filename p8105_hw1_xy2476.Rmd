---
title: "Homework 1"
author: Rio Yan
output: github_document
---

This is my solution to HW1. 

```{r}
library(tidyverse)
```


## Problem 1

Create a data frame with the specified elements.

```{r create_df}
prob1_df = 
  tibble(
    samp = rnorm(10),
    samp_gt_0 = samp > 0,
    char_vec = c("a", "b", "c", "d", "e", "f", "g", "h", "i", "j"),
    factor_vec = factor(c("low", "low", "low", "mod", "mod", "mod", "mod", "high", "high", "high"))
)
```

Take the mean of each variable in my data frame.

```{r mean_pull_var}
mean(pull(prob1_df, samp))
mean(pull(prob1_df, samp_gt_0))
mean(pull(prob1_df, char_vec))
mean(pull(prob1_df, factor_vec))
```

I can take the mean of numbers and logical but not character or factor.

```{r try_numeric, eval = FALSE}
as.numeric(pull(prob1_df, samp))
as.numeric(pull(prob1_df, samp_gt_0))
as.numeric(pull(prob1_df, char_vec))
as.numeric(pull(prob1_df, factor_vec))
```

Try converting logical vector and multiply the random sample.
```{r try_factor_numeric, eval = FALSE}
as.numeric(pull(prob1_df, samp_gt_0)) * pull(prob1_df, samp)
as.factor(pull(prob1_df, samp_gt_0)) * pull(prob1_df, samp)
as.numeric(as.factor(pull(prob1_df, samp_gt_0))) * pull(prob1_df, samp)
```


## Problem 2

Load the penguins dataset.

```{r load_data}
data("penguins", package = "palmerpenguins")
```

The variable names in the dataset includes `r names(penguins)`.

The sample size of the dataset contains `r nrow(penguins)` penguins.

There are three penguin species in the dataset: `r penguins[1, 1]`, `r penguins[153, 1]`, and `r penguins[281, 1]` and the data were collected from three islands in `r penguins [1, 2]`, `r penguins [21, 2]`, and `r penguins [85, 2]`.

The dataset contains `r nrow(penguins)` rows and `r ncol(penguins)` columns.

The mean flipper length of the penguins in the dataset is `r mean(pull(penguins, flipper_length_mm), na.rm = TRUE)`mm.



Make the scatterplot.

```{r plot_scatter}
library(ggplot2)

ggplot(penguins, aes(bill_length_mm, flipper_length_mm, color = species)) + geom_point()

ggsave("scatter_plot.pdf", height = 4, width = 6)

```

